...
def preprocess_image(cv2im, mean, std, resize_im=False):
    """
...
def recreate_image(im_as_var, mean, std, blur = False):
    """
    ...
    """
    reverse_mean = mean
    reverse_mean[:] = [-1 * x for x in reverse_mean]
    reverse_std = std
    reverse_std[:] = [1 / x for x in reverse_std]
    ...
    if blur:
        recreated_im = gaussian_filter(recreated_im, sigma = blur)
